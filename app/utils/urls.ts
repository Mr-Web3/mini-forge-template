...
// Coinbase Developer Platform Project ID (from environment or config)
const CDP_PROJECT_ID = process.env.NEXT_PUBLIC_ONCHAINKIT_PROJECT_ID || 'a353ad87-5af2-4bc7-af5b-884e6aabf088';
...
export function generateOnrampURL(params: OnrampURLParams): string {
  ...
  // If using session token with OnchainKit utility
  if (sessionToken) {
    try {
      return getOnrampBuyUrl({
        sessionToken: sessionToken,
        presetFiatAmount: numericAmount > 0 ? numericAmount : undefined,
        fiatCurrency: paymentCurrency || 'USD',
        defaultNetwork: network,
        defaultAsset: asset,
      });
    } catch (error) {
      console.warn("Error using OnchainKit utility, falling back to manual URL generation:", error);
    }
  }
  ...
  if (sessionToken) {
    ...
    if (redirectUrl) {
      ...
      queryParams.append("redirectUrl", typeof window !== 'undefined' ? window.location.origin + '/onramp' : '');
    }
...
export function generateOfframpURL(params: OfframpURLParams): string {
  try {
    ...
    if (sessionToken) {
      ...
      // Add redirect URL
      queryParams.append("redirectUrl", redirectUrl || (typeof window !== 'undefined' ? window.location.origin + '/offramp' : ''));
    }
    ...
    return `https://pay.coinbase.com/v3/sell/input?appId=${CDP_PROJECT_ID}&partnerUserId=anonymous&assets=["USDC"]&addresses={"0x4315d134aCd3221a02dD380ADE3aF39Ce219037c":["ethereum"]}&redirectUrl=${encodeURIComponent(typeof window !== 'undefined' ? window.location.origin + '/offramp' : '')}`;
  }
}
